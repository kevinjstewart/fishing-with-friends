...
async function verifySharedSessionRecovery({ browser, base, check, recordConsoleError }) {
  ...
  await page.evaluate(({ itemId, rodId }) => {
    const app = document.querySelector("game-app");
    if (!app) throw new Error("Game app is missing.");
    const options = { bubbles: true, composed: true };
    app.dispatchEvent(new CustomEvent("ui:purchase", { ...options, detail: { itemId, quantity: 1 } }));
    app.dispatchEvent(new CustomEvent("ui:select-equipment", { ...options, detail: { rodId } }));
  }, { itemId: state.catalog.baits[0].id, rodId: state.activeEquipment.rodId });
...
async function verifyCatchDecisionRetry({ browser, base, check, recordConsoleError }) {
  ...
  await page.waitForSelector(".locations-list .location-card", { timeout: 20_000 });
  await page.evaluate(() => {
    const hook = window.__FISHING_PHASE0__;
    if (!hook) throw new Error("Phase 0 result test hook is missing.");
  ...
  check("catch decision retry succeeds once", decisionRequests === 2 && completionRequests === 1 && (await page.locator(".locations-list .location-card").count()) > 0, `${decisionRequests} decision requests; one completion; returned to lakes`);
  await page.close();
}
...
async function verifySellAllPartialReconciliation({ browser, base, check, recordConsoleError }) {
  ...
  await page.evaluate(() => {
    const app = document.querySelector("game-app");
    if (!app) throw new Error("Game app is missing.");
    const event = () => new CustomEvent("ui:sell-all", { bubbles: true, composed: true });
  ...
  const status = await page.locator('.toast[data-state="ready"]').textContent();
