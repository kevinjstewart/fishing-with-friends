...
export type EncounterEvent =
  ...
  | { type: "DECISION_FAILED"; message: string }
  | { type: "PHASER_AMBIENT" }
  | { type: "RETRY" }
...
export function encounterReducer(state: EncounterState, event: EncounterEvent): EncounterState {
  switch (event.type) {
    ...
    case "START_SUCCEEDED":
      ...
      return { ...state, phase: "fighting", encounter: event.encounter, result: null, decision: null, ambientReady: false, error: undefined };
    case "START_FAILED":
      if (state.phase !== "starting") return state;
    ...
    case "COMPLETE_SUCCEEDED":
      ...
      return state.ambientReady
        ? { ...state, phase: "result", result: event.result, error: undefined }
        : { ...state, result: event.result, error: undefined };
    case "COMPLETE_FAILED":
      if (state.phase !== "resolving") return state;
      return recoverableError(state, { operation: "completion", message: event.message });
    ...
    case "DECISION_SUCCEEDED":
      ...
      return { ...state, phase: "decision-result", decision: event.result, error: undefined };
    case "DECISION_FAILED":
      if (state.phase !== "deciding") return state;
      return recoverableError(state, { operation: "decision", message: event.message });
    case "PHASER_AMBIENT":
      if (state.phase !== "resolving") return state;
    ...
    case "RETRY":
      ...
      if (canRetry(state, "completion")) return { ...state, phase: "resolving", error: undefined };
