...
export function icon(name: IconName, className = "icon"): DirectiveResult {
  const paths = ICON_PATHS[name].map((path) => `<path d="${path}"></path>`).join("");
  return unsafeSVG(`<svg class="${className}" viewBox="0 0 24 24" aria-hidden="true">${paths}</svg>`);
}
